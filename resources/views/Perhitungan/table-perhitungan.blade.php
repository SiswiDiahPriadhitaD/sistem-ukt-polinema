...
@section('content')
    <section class="section">
        <div class="section-header d-flex justify-content-between align-items-center">
            <h1>List Mahasiswa Hasil Perhitungan</h1>
            <a class="btn btn-primary" href="{{ route('mahasiswa-perhitungan.index') }}">Kembali</a>
        </div>
    ...
    @include('Perhitungan.modal-edit-status')
    ...
    <script>
        ...
        function showStatusModal(id) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#status_verifikasi_perhitungan').val(data.status_verifikasi_perhitungan).trigger(
                        'change');
        ...
        $(document).ready(function() {
            ...
            var table = $('.mahasiswa-data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "/perhitungan/mahasiswa/perhitungan-table/" + periode_id,
                    type: 'GET',
                    data: {
                ...
                columns: [{
                    ...
                    {
                        data: 'status_verifikasi_perhitungan',
            ...
            $('.mahasiswa-data-table tbody').on('click', 'td button', function() {
                ...
                if (row.child.isShown()) {
                    ...
                    $.ajax({
                        url: "{{ route('mahasiswa-perhitungan.details', ':id') }}".replace(':id',
                            id),
                        method: 'GET',
                        success: function(data) {
                            row.child(data).show();
            ...
            $('#statusVerifikasiForm').on('submit', function(event) {
                ...
                var status = $('#status_verifikasi_perhitungan').val();
                $.ajax({
                    url: "{{ route('mahasiswa.update-status-pehitungan', ':id') }}".replace(':id',
                        id),
                    method: "PUT",
                    data: {
                        _token: '{{ csrf_token() }}',
                        status_verifikasi_perhitungan: status
                    },
                    success: function(data) {
                        if (data.success) {
            ...
            function setValidationErrors(errors, formType) {
                if (errors.status_verifikasi_perhitungan) {
                    $('#status_verifikasi_perhitungan').addClass('is-invalid');
                    $('#status_verifikasi_perhitungan_error').text(errors.status_verifikasi_perhitungan[0]);
                }
            }
